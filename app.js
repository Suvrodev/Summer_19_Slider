...
//    console.log('This Length: '+PreviousBookmarked.length)
   ///New.................................
   ...
   // console.log('This is Retrieve Function')
    ...
    PreviousBookmarked.forEach(student=>{
       ...
       // console.log(x_Picture)
        console.log('Ago Push')
        ImageArray.push(x_Picture)
        console.log('Length of Image Array: '+ImageArray.length)

        SelectedImage.innerHTML +=`
        <img onclick="outputImage('${x_Picture}')" class="imgsm" src="${student.Image}" >
      `
    })
}
...
const outputImage=(url)=>{
  //  console.log('Image url: '+url)
    const PreviousBookmarked=JSON.parse(localStorage.getItem('Summer19'));
    if(PreviousBookmarked){
        const isThisStudent=PreviousBookmarked.find(std=>std.Image===url)
        if(isThisStudent){
            console.log('Contain')
            const restOfThem=PreviousBookmarked.filter(std=>std.Image!==url)
            localStorage.setItem('Summer19',JSON.stringify(restOfThem))
            Retrieve_Students()
        }else{
            // Students.push(...PreviousBookmarked,Student)
            // localStorage.setItem('Summer19',JSON.stringify(Students))
